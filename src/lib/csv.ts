...
/**
 * 進捗付き非同期パース。大きな CSV をブラウザでパースする際、UI を固めずに
 * 「処理済み文字数 / 総文字数」を onProgress で通知する (importView の進捗バー用)。
 * ※ ロジックは parseCsv と同一に保つこと (片方を直したら両方直す)。
 */
export async function parseCsvAsync(
  ...
  let i = 0;
  const n = text.length;
  const CHUNK = 500_000; // この文字数ごとに UI へ制御を返す
  let nextYield = CHUNK;
